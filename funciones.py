...
def check_eventos(screen,settings,ship,bullets,aliens,stats,boton_play,tablero):
    '''
    ...
    for event in pygame.event.get():  #lista con los eventos
        ...
        if event.type==pygame.KEYDOWN:
            ...
            if event.key==pygame.K_s:
                    ...
                    reset_juego(screen,settings,ship,bullets,aliens,stats,tablero)               
...
def actualiza_screen(screen,settings,ship,bullets,aliens,stats,boton_play,tablero):
    ...
    if stats.juego_activo:
        ...
        colision=pygame.sprite.groupcollide(bullets,aliens,False,True)  #remueve la interseccion de sprites en ambos grupos
        if colision:
...
def crear_flota(settings,screen,ship,aliens):
    ...
    espacio_disponible_x=settings.screen_width-2*alien_width
    numero_aliens_x=espacio_disponible_x//(2*alien_width)

    espacio_disponible_y=settings.screen_height-ship.rect.height-3*alien.rect.height
...
def cambiar_direccion_flota(settings,aliens):
    for alien in aliens.sprites():
        alien.rect.y+=settings.fleet_speed_down
...
def reset_juego(screen,settings,ship,bullets,aliens,stats,tablero):
    stats.reset_stats()
    aliens.empty()
    bullets.empty()
    crear_flota(settings,screen,ship,aliens)
